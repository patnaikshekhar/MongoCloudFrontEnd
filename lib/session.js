...
const SESSION_COOKIE = require('./constants.json').SESSION_COOKIE
...
const checkValidSession = (sessionId, callback) => {
  DB.get(SESSION_TABLE_NAME, { sessionId }, (err, value) => {
    if (value) {
      if (value.sessionStart) {
        const timeout = config['SessionTimeout']
        if (Date.now() - (new Date(value.sessionStart)) <= timeout) {
    ...
    DB.put(USER_TABLE_NAME, userDoc, (err, value) => {
      if (err) {
        ...
        DB.put(SESSION_TABLE_NAME, sessionDoc, (err, value) => {
          response.cookie(SESSION_COOKIE, sessionDoc.sessionId)
          callback(err, null)
        })
      }
    })
